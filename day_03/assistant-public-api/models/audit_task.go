...
type IAuditTask interface {
	...
	List(ctx context.Context, filter map[string]any) ([]*entities.AuditTask, error)
}

type AuditTask struct {
}

func (AuditTask) Create(ctx context.Context, data *entities.AuditTask) (int64, error) {
...
func (AuditTask) UpdateField(ctx context.Context, userID, id int64, field string, value any) error {
	return clients.MySQLClient.WithContext(ctx).Model(&entities.AuditTask{}).Session(&gorm.Session{FullSaveAssociations: true}).Where("id = ?", id).Update(field, value).Error
}

func (AuditTask) Read(ctx context.Context, id int64) (*entities.AuditTask, error) {
	result := &entities.AuditTask{}
...
func (AuditTask) Update(ctx context.Context, data *entities.AuditTask) error {
...
func (AuditTask) Delete(ctx context.Context, id int64) error {
	return clients.MySQLClient.Transaction(func(tx *gorm.DB) error {
		return tx.WithContext(ctx).Model(&entities.AuditTask{}).Session(&gorm.Session{FullSaveAssociations: true}).Where("id = ?", id).Delete(&entities.AuditTask{}).Error
	})
}

func (AuditTask) List(ctx context.Context, filters map[string]any) ([]*entities.AuditTask, error) {
	result := []*entities.AuditTask{}
	db := clients.MySQLClient.WithContext(ctx).
		Preload("Project").
		Preload("ParentTask").
